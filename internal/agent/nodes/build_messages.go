...
// BuildFunc 是"历史 + 原始 Query + 用户 ID + 人设参数快照 → 完整消息列表"的契约。
...
// 另外的开关参数。
type BuildFunc func(history []*schema.Message, query, userID string, snap stats.Snapshot) ([]*schema.Message, error)
...
func NewBuildMessages(build BuildFunc) *compose.Lambda {
	...
	return compose.InvokableLambda(func(_ context.Context, st *flow.State) (*flow.State, error) {
		msgs, err := build(st.History, st.In.Query, st.In.UserID, st.Stats)
		if err != nil {

// Package nodes 的 build_messages.go 实现"把长期记忆 + 话题锚点 + 历史 + 用户 Query 组装成
...
type BuildFunc func(history []*schema.Message, query, userID string, affinity, mood int, memory string, casualTopics []string, groupBackground string) ([]*schema.Message, error)

// NewBuildMessages 构造 buildMessages 节点。
//
// build 必须非 nil——若为 nil 则在调用期才爆炸，反而不如启动期直接 panic 更"快失败"。
// stats 平铺字段、长期记忆、闲聊话题锚点与群聊背景都由上游节点提前写入 State；本节点只消费已有值，
// 不再触碰 Redis。这样"对话前读取上下文"与"prompt 组装"边界清晰。
func NewBuildMessages(build BuildFunc) *compose.Lambda {
	...
	return compose.InvokableLambda(func(_ context.Context, st *flow.State) (*flow.State, error) {
		msgs, err := build(st.History, st.In.Query, st.In.UserID, st.Affinity, st.Mood, st.Memory, st.CasualTopics, st.GroupBackground)
		if err != nil {

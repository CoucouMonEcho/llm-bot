...
type personaFile struct {
	Persona struct {
		...
		Examples    []PersonaExample `yaml:"examples"`
		Rules       []string         `yaml:"rules"`
	} `yaml:"persona"`
	Guardrails string `yaml:"guardrails"`
}

// PersonaExample 是人设 prompt 的 few-shot 示例。
//
// 主聊天链路的输入是真实用户消息，因此这里保留 user -> assistant 的映射关系；
// 运行期只会渲染到 system prompt 中，不会伪造成历史消息。
...
type Persona struct {
	...
	//   persona.description + examples + "规则:\n" + rules + "\n" + guardrails
...
func LoadPersona(path string) (*Persona, error) {
	...
	examples, err := normalizePersonaExamples(pf.Persona.Examples)
	if err != nil {
		return nil, err
	}

	// Step 2: 把 description + examples + rules + guardrails 拼成 SystemPrompt。
	// 顺序刻意把 guardrails 放在最后，使其在模型注意力中具有"最近优先"的效果。
	// examples 放在 description 后、rules 前，是为了贴近旧 prompt 中"人设说明后
	// 立刻给自然回复示例"的顺序，降低迁移带来的行为偏移。
	var sb strings.Builder
	sb.WriteString(strings.TrimSpace(pf.Persona.Description))
	appendPersonaExamples(&sb, examples)
...
func normalizePersonaExamples(examples []PersonaExample) ([]PersonaExample, error) {
	out := make([]PersonaExample, 0, len(examples))
	for i, example := range examples {
		example.User = strings.TrimSpace(example.User)
		example.Assistant = strings.TrimSpace(example.Assistant)
		if example.User == "" && example.Assistant == "" {
			continue
		}
		if example.User == "" || example.Assistant == "" {
			return nil, fmt.Errorf("agent: persona.examples[%d] must contain both user and assistant", i)
		}
...
func appendPersonaExamples(sb *strings.Builder, examples []PersonaExample) {
	if len(examples) == 0 {
		return
	}
	sb.WriteString("\n\n## 自然回复示例（只学风格，不要机械复读）\n")
	sb.WriteString("不要输出标题、编号或字段名，只学习输入到回复的映射、语气和长度。\n")
	for i, example := range examples {
		fmt.Fprintf(sb, "\n### 示例 %d\n", i+1)
		fmt.Fprintf(sb, "- 用户输入：%s\n", example.User)
		fmt.Fprintf(sb, "- 推荐回复：%s\n", example.Assistant)
	}
}
...
// casualTopics 是全局闲聊话题锚点；groupBackground 是 loadContext 节点针对群聊会话渲染好的"刚才群里在聊什么"
// 背景块（私聊或缓存关闭时为空）。本方法不感知 Redis，只负责把这几段上下文
// 用双换行隔离地拼到 SystemPrompt 末尾，并在最后补一条动态护栏。
//
// 群聊背景块只在群聊会话出现，且与长期记忆/话题锚点/状态行/动态护栏一起按
...
// 后委托给 PromptLine 渲染。长期记忆、话题锚点与群聊背景则保持纯文本，避免把"记忆格式"
// 扩散到 agent 之外。
//
// 注意不要写回 p.SystemPrompt：那是启动期固化的只读快照，多 goroutine 共享；
// 这里每次调用都在栈上用 strings.Builder 构造一份新的 system content。
func (p *Persona) BuildMessages(history []*schema.Message, query, userID string, affinity, mood int, memory string, casualTopics []string, groupBackground string) ([]*schema.Message, error) {
	snap := stats.Snapshot{Affinity: affinity, Mood: mood}
	line := snap.PromptLine()
	memory = strings.TrimSpace(memory)
	casualTopics = normalizeCasualTopics(casualTopics)
	...
	if len(casualTopics) > 0 {
		sb.WriteString("\n\n")
		sb.WriteString("最近可延续的闲聊话题锚点（仅供自然承接，不要逐字复述或强行提起）：\n")
		for _, topic := range casualTopics {
			sb.WriteString("- ")
			sb.WriteString(topic)
			sb.WriteByte('\n')
		}
	}
	// 群聊背景紧跟长期记忆与话题锚点之后、状态行之前：让模型先建立"这是一个用户的画像 +
	// 可延续话题 + 群里的氛围"，再读到"当前时间和你与他的关系"，最后才看到动态护栏与本轮 Query。
	if groupBackground != "" {
		sb.WriteString("\n\n")
...
func normalizeCasualTopics(topics []string) []string {
	if len(topics) == 0 {
		return nil
	}
